...
func (s *serverCRUD) Subscribe(e *emptypb.Empty, stream grpc.ServerStreamingServer[protobufStorage.TodoEvent]) error {
	// Subscribe to events from the storage
	eventChan := s.todoStore.Subscribe()
	...
	for {
		...
		case event, ok := <-eventChan:
			...
			// Convert storage.TodoEvent to protobufStorage.TodoEvent
			pbEvent := &protobufStorage.TodoEvent{
				T: &protobufStorage.Todo{
					Task:      event.Entry.Task,
					Completed: event.Entry.Completed,
				},
				Action: event.Action,
			}
